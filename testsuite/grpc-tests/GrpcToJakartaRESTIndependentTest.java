...
//@RunAsClient
public class GrpcToJakartaRESTIndependentTest {
    ...
    static Archive<?> deploy() throws Exception {
        try {
            ...
            Archive<?> ar = ShrinkWrap.create(WebArchive.class, GrpcToJakartaRESTIndependentTest.class.getSimpleName() + ".war")
    ...
    static void accessServletContexts() {
        try (
                Client client = ClientBuilder.newClient(); //http://localhost:8080/grpc-test/grpcToJakartaRest/grpcserver/context
                var response = client
                        .target("http://localhost:8080/grpc-test/grpcToJakartaRest/grpcserver/context")
                        .request()
                        .get()) {
    ...
    void testArraysInts2(CC1ServiceBlockingStub stub) throws Exception {
        ...
        try {
            //           ArrayStuff ass = (ArrayStuff) translator.translateFromJavabuf(CC1_proto.dev_resteasy_grpc_example___ArrayStuff.newBuilder().build());
            //            Object oo = ArrayUtility.getArray(translator, ah, "CC1");
    ...
    //            ArrayStuff expected = new ArrayStuff(false);
    ...
    //            Assert.assertEquals(new ArrayStuff(true), arrayStuff);
    //        } catch (StatusRuntimeException e) {
    //            try (StringWriter writer = new StringWriter()) {
    //                e.printStackTrace(new PrintWriter(writer));
    //                Assert.fail(writer.toString());
